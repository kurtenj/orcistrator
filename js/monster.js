...
export function displayMonsterStatBlock(monster) {
  ...
  if (!monsterData) {
    elements.activeStatBlockElement.innerHTML =
      '<div class="turn-info">Monster details not available.</div>';
    return;
  }
  ...
  const isCurrentTurn =
    getCombatStarted() && getParticipants()[getCurrentTurnIndex()]?.id === monster.id;
        ...
        <div class="stat-block" data-monster-id="${monster.id}" data-monster-hp="${monster.currentHp}">
            ...
            <div class="stat-block-content">
                ...
                <div class="grid grid-cols-3 gap-2 mb-4">
                    ...
                    <div class="text-sm"><strong>Speed</strong> ${Object.entries(
                      monster.speed || monsterData.speed
                    )
                      .map(([type, speed]) =>
                        type === 'walk' ? `${speed} ft.` : `${type} ${speed} ft.`
                      )
                      .join(', ')}</div>
                </div>
  ...
  if (isCurrentTurn && actions && actions.length > 0) {
    const actionButtons = elements.activeStatBlockElement.querySelectorAll(
      '[data-action="monster-action"]'
    );
...
const _handleMonsterActionSelection = (_monster, _target, _monsterActions, _event) => {
  // ... existing code ...
};
