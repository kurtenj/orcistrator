...
import { addParticipant, getParticipants, getMonsterData, removeParticipant, getCurrentTurnIndex, getCombatStarted } from './state.js';
...
export function handleAddRandomPC(count = 1) {
    ...
    const characterNames = [
        "Aragorn", "Gandalf", "Legolas", "Gimli", "Frodo", "Samwise", "Meriadoc", "Peregrin",
        "Boromir", "Faramir", "Éowyn", "Éomer", "Théoden", "Galadriel", "Elrond", "Arwen",
        "Bilbo", "Thorin", "Balin", "Dwalin", "Fíli", "Kíli", "Glóin", "Óin", "Dori", "Nori",
        "Ori", "Bifur", "Bofur", "Bombur", "Thranduil", "Tauriel", "Bard", "Beorn", "Radagast",
        "Saruman", "Gollum", "Smaug", "Denethor", "Celeborn", "Haldir", "Glorfindel", "Círdan",
        "Elendil", "Isildur", "Gil-galad", "Celebrimbor", "Thingol", "Lúthien", "Beren"
    ];
    ...
    for (let i = 0; i < count; i++) {
        ...
        // Log to console
        console.log(`Added random PC: ${randomName} (Initiative: ${randomInitiative}, AC: ${randomAC})`);
    }
            ...
            try {
                ...
                if (monsterData) {
                    ...
                    for (let i = 0; i < count; i++) {
                        ...
                        if (monsterData.armor_class && Array.isArray(monsterData.armor_class) && monsterData.armor_class.length > 0) {
                            armorClass = monsterData.armor_class[0].value;
                            console.log('Monster armor class extracted:', armorClass);
                        } else {
                            console.warn('Could not extract armor class from monster data:', monsterData.armor_class);
                        }
